...
class DefaultController extends Controller
{
    ...
    public function beforeAction($action)
    {
        ...
        {
            Yii::$app->session['debug_log'] = Yii::$app->request->get('debug');
        }
    ...
    protected function getManifest($forceReload = false)
    {
        if ($this->_manifest === null || $forceReload)
        {
            if($callback = $this->module->targetRedis)
            {
                $redis = $callback();
                // 判断是否开启redis存储
                $content = $redis->hget(Yii::$app->request->get('debug', Yii::$app->session['debug_log']) . '_debug_log', 'index.data');
                if(empty($content))
                {
                    $this->_manifest = [];
                }
        ...
        for ($retry = 0; $retry <= $maxRetry; ++$retry) {
            ...
            if (isset($manifest[$tag])) {
                if($callback = $this->module->targetRedis)
                {
                    $redis = $callback();
                    // 判断存储介质是否为redis
                    $data = unserialize($redis->hget(Yii::$app->request->get('debug', Yii::$app->session['debug_log']) . '_debug_log', $tag));
                }
                else
                {

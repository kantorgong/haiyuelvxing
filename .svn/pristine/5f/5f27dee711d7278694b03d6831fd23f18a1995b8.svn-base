...
class LogTarget extends Target
{
    ...
    public function export()
    {
        ...
        if ($callback = $this->module->targetRedis)
        {
            $redis = $callback();
            if(Yii::$app->request->serverName)
            {
                $redis_key = Yii::$app->request->serverName . '_debug_log';
            }
            else
            {
                $redis_key = Yii::$app->name . '_debug_log';
            }
            // 判断存储是否放在redis中
            $redis->hset($redis_key, $this->tag, serialize($data));
            // 更新索引文件
            $manifest = $redis->hget($redis_key, 'index.data');
            ...
            $redis->hset($redis_key, 'index.data', serialize($manifest));
        }
        else
        {

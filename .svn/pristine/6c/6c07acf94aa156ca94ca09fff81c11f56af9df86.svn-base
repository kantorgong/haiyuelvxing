...
use components\XyXy;
class BtestController extends \frontend\modules\activity\components\Controller
{
    // 设置redis 键的前缀
...
/*        if (!$this->_isWechat())
    ...
    public function actionAjaxCard()
    {
        ...
        return $this->_successReturn(array('money'=>$prize,'isClick'=>Yii::$app->redis->hget($this->__getKey('userPrize_num_' . date('Ymd')), $openId)));
    }
    ...
    /*
    // @todo 测试
    public function actionAjaxCardt()
    {
        // @todo 各种权限限制条件校验 IP是否为湖南等等...
        $openId = $this->__getOpenId();
        $openId = md5(time() . mt_rand(1,99999));
        // 校验用户是否有权限刮卡
        $result = $this->__checkScratcht($openId);
        if($result !== true)
        {
            return $this->_failReturn($result);
        }
    ...
    // @todo 测试
    private function __checkScratcht($openId)
    {
        // 判断用户是否刚从答题页面跳入过来
        $userFrom = Yii::$app->redis->hget($this->__getKey('uAnswer_token'), $openId);
        if($userFrom != 'end')

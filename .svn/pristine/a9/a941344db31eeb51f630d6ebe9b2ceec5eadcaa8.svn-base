...
class UploadFile {//类定义开始
    ...
    private function save($file) {
        ...
        if($this->thumb && in_array(strtolower($file['extension']),array('gif','jpg','jpeg','bmp','png'))) {
            ...
            if(false !== $image) {
                ...
                for($i=0,$len=count($thumbWidth); $i<$len; $i++) {
                    ...
                    \components\helper\Image::thumb($filename,$thumbPath.$thumbname.'.'.$thumbExt,'jpeg',$thumbWidth[$i],$thumbHeight[$i],true);
                }
        ...
        //水印
        if ($this->water && $this->water_img)
        {
            \components\helper\Image::water($filename, $this->water_img, null, $this->water_alpha);
        }
        if($this->zipImags) {
            // TODO 对图片压缩包在线解压

...
use components\XyXy;
class AnswerController extends \frontend\modules\activity\components\Controller
{
    // 设置redis 键的前缀
...
//        $res = \Yii::$app->redis->expire($this->__getKey('userPrize_num_' . date('Ymd')), 1);
        $res = \Yii::$app->redis->hdel($this->__getKey('userPrize_num_' . date('Ymd')), $openId);
        $shareNum = Yii::$app->redis->hdel($this->__getKey('userShare_num_' . date('Ymd')), $openId);
        // 删除分享次数
